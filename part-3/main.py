...
def load_sample_data(paths: tuple[str, str, str, str, str]) -> list:
    """
    Load sample data from CSV files and convert it to JSON format.

    Args:
        paths (tuple): A tuple of file paths to the CSV files containing sample data.

    Returns:
        list: A list of dictionaries representing the rows in the CSV files.
    """
    data = []
    for path in paths:
        with open(path, "r") as f:
            ...
            for row in reader:
                # Convert each row to a JSON-compatible dictionary
                json_row = {key: value for key, value in row.items()}
...
if __name__ == "__main__":
    ...
    print(f"Loaded {len(data)} records from sample data files.")
